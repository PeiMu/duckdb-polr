...
#  fi
#  echo $command
#  echo -ne "${command}" | duckdb ./ssb_skew_$2_polr.db
#done
...
for i in $(eval echo {1.."${iteration}"}); do
  for sql in "${dir}"/*.sql; do
    ...
    echo -ne ".read ${sql}" | duckdb ./ssb_skew_$2_polr.db 2>&1|tee -a ${log_name};
  done
done
