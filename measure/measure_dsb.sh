...
cd ../ && make clean && GEN=ninja VERBOSE=1 make >> compile.log 2>&1 && cd measure && bash ./hyperfine_in_mem_dsb.sh official nan $1
...
cd ../ && make clean && GEN=ninja VERBOSE=1 make >> compile.log 2>&1 && cd measure && bash ./hyperfine_in_mem_dsb.sh polr nan $1
